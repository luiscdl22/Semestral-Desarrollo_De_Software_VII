...
<div class="admin-header-innovative">
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
        <div>
            <h2><i class="bi bi-clipboard-data"></i>Gestión de Encuestas</h2>
            <p>Administra las encuestas promocionales del sistema</p>
        </div>
        <div>
...
<div class="admin-actions">
    <a href="<?= APP_URL ?>/admin/surveys/create" class="btn-admin-innovative">
...
<div class="table-responsive">
    <table class="table-admin-innovative" id="surveysTable">
        <thead>
            <tr>
        ...
        <tbody>
            <?php foreach ($surveys as $survey): ?>
                <?php $opts = json_decode($survey->options ?? '[]', true) ?: []; ?>
                <tr>
                    <td>#<?= $survey->id ?></td>
                    <td><strong><?= htmlspecialchars($survey->question) ?></strong></td>
                    <td>
                        <div class="survey-options-preview">
                            <?php foreach ($opts as $opt): ?>
                                <span class="survey-option-chip"><?= htmlspecialchars($opt) ?></span>
                            <?php endforeach; ?>
                        </div>
                    </td>
                    <td style="text-align:center; white-space:nowrap;">
                        <a href="<?= APP_URL ?>/admin/surveys/edit/<?= $survey->id ?>" class="btn-admin-innovative btn-admin-innovative-sm btn-admin-innovative-warning">
                            <i class="bi bi-pencil"></i>
                        </a>
                        <form method="POST" action="<?= APP_URL ?>/admin/surveys/delete/<?= $survey->id ?>" class="delete-form-inline" onsubmit="return confirm('¿Eliminar esta encuesta?')">
...
<div class="text-center" style="font-family: var(--font-mono); font-size: 0.7rem; color: var(--text-light);">
    ...
    Solo administradores y armadores pueden gestionar encuestas
</div>